...
function castLatLngToObj(latlngLit) {
...
function animate(d) {
    ...
    if (d > eol || !p) {
        alert('Trip completed! Distance: '+ (d / 1609.344).toFixed(2) + ' miles');
        return;
    }
    ...
    if (poly.GetIndexAtDistance(d) > lastVertex) {
        ...
        angle = bearing(paths[lastVertex - 1], paths[lastVertex]);
        marker.plot(angle);
    }
...
function initCar() {
    ...
    window.map.panTo(pathDirections[0]);
    window.map.setZoom(17);

    /* Start Marker */
    new google.maps.Marker({
        position: pathDirections[0],
        map: window.map
    });

    /* End Marker */
    new google.maps.Marker({
        position: pathDirections[pathDirections.length - 1],
        map: window.map
    });

    marker = new CanvasMarker({
        position: castLatLngToObj(pathDirections[0]),
        id: 'carcanvas',
        image: img,
    ...
    var p0 = castLatLngToObj(pathDirections[0]);
    var p1 = castLatLngToObj(pathDirections[1]);
    angle = bearing(p0, p1);
